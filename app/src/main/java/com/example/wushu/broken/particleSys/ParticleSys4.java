...
public class ParticleSys4 implements Particleable{
    ...
    private float bigc_x0,bigc_y0; // 大圆的位置

    public ParticleSys4(@NonNull ArrayList<Particle> ps, float x, float y, float init_r, float step_r, float part_r)
    {
        activate = new ArrayList<>();
        activate_r = init_r;
        this.step_r = step_r;
        this.part_r = part_r;
        bigc_x0 = ps.get(0).pos.x - part_r;
        bigc_y0 = ps.get(0).pos.y - part_r;
    ...
    public void update(float delatiem) {
        for(int i = 0;i < activate.size() ;i++)
        {
            if(activate.get(i))
                continue;
            if(bigc_x0 + activate_r >  myPs.get(i).pos.x + part_r && bigc_y0 + activate_r > myPs.get(i).pos.y + part_r )
            {
                activate.set(i,true);
            }
        }
